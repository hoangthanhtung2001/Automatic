...
def show_image_cv2(screenshot_np, pt, w, h):
    hide_taskbar()
    hide_other_windows()
    root = tk.Tk()
...
def wait_for_target_appear(target_image, interval=1, timeout=None):
    ...
    while True:
        ...
        if  check_image_on_screen(target_image,sleep_time=1,show=False) and timeout is not None and time.time() - start_time< timeout :
            ...
            return False
        elif not check_image_on_screen(target_image,sleep_time=1,show=False) and timeout is not None and time.time() - start_time>= timeout :
